...
int PubSubClient::connect(char *id, char *username, char *password, char* willTopic, uint8_t willQos, uint8_t willRetain, char* willMessage) {
   if (!connected()) {
      if (_client.connect(this->ip, this->port)) {
         ...
         if (willTopic) {
           flags |= willRetain << 5 | willQos << 3 | 0x04;
         }
         buffer[length++] = flags;
         buffer[length++] = 0;
