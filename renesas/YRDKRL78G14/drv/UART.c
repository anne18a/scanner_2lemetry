 ...
 *-------------------------------------------------------------------------*/
    ...
    #error "UART0_RX_BUFFER_SIZE must be defined in platform.h"
#endif
#ifndef UART0_TX_BUFFER_SIZE
    #error "UART0_TX_BUFFER_SIZE must be defined in platform.h"
#endif
...
#define UART0_RE_INTERRUPT_PRIORITY 1U  // Low
    ...
    #error "UART1_RX_BUFFER_SIZE must be defined in platform.h"
#endif
#ifndef UART1_TX_BUFFER_SIZE
    #error "UART1_TX_BUFFER_SIZE must be defined in platform.h"
#endif

#ifndef UART1_TX_INTERRUPT_PRIORITY
#define UART1_TX_INTERRUPT_PRIORITY 1U   // Low
#endif

#ifndef UART1_RX_INTERRUPT_PRIORITY
#define UART1_RX_INTERRUPT_PRIORITY 1U  // Low
#endif

#ifndef UART1_RE_INTERRUPT_PRIORITY
#define UART1_RE_INTERRUPT_PRIORITY 1U  // Low
#endif
...
/* Transmit FIFO buffer */
static uint8_t G_UART1_RXBuffer[UART1_RX_BUFFER_SIZE];
static uint16_t G_UART1_RXIn = 0;
static uint16_t G_UART1_RXOut = 0;

/* Transmit FIFO buffer */
static uint8_t G_UART1_TXBuffer[UART1_TX_BUFFER_SIZE];
static uint16_t G_UART1_TXIn = 0;
static uint16_t G_UART1_TXOut = 0;
static volatile bool G_UART1_TX_Empty;

static volatile T_SAUStatusError G_UART1_LastError = NONE;
static uint16_t G_UART1_ErrorCount = 0;
...
void RL78G14RDK_UART_Start(uint32_t UART0_baud, uint32_t UART1_baud)
{
    ...
    G_UART1_RXIn = G_UART1_RXOut = 0;
    G_UART1_TXIn = G_UART1_TXOut = 0;
    G_UART1_TX_Empty = true;      
    ...
    Create_UART0(UART0_baud);
    Create_UART1(UART1_baud);
}

/*---------------------------------------------------------------------------*
 * Routine:  Create_UART0
 *---------------------------------------------------------------------------*
 * Description:
 *      Each time the timer fires off an interrupt, this routine catches it
 *      and passes it to the given interrupt service routine.
...
{
    /* disable UART1 receive and transmit */
...
bool UART1_ReceiveByte(uint8_t *aByte)
{
    ...
    /* Check to see if any bytes have been placed in the FIFO and */
    /* are waiting to be pulled out. */
    if (G_UART1_RXIn != G_UART1_RXOut) {
        *aByte = G_UART1_RXBuffer[G_UART1_RXOut++];
        if (G_UART1_RXOut >= UART1_RX_BUFFER_SIZE)
            G_UART1_RXOut = 0;
        found = true;
    }
...
bool UART1_SendByte(uint8_t aByte)
{
    ...
    if (G_UART1_TX_Empty) {
        ...
        /* Calculate where the next in position is in the FIFO */
        next = G_UART1_TXIn+1;
        if (next >= UART1_TX_BUFFER_SIZE)
            next = 0;

        /* Is there room in the transmit FIFO? */
        if (next != G_UART1_TXOut) {
            /* There is room, place a byte in the FIFO */
            G_UART1_TXBuffer[G_UART1_TXIn] = aByte;
            G_UART1_TXIn = next;
            placed = true;
        } else {
            /* There is no room in the FIFO, return the failed case */
...
void UART1_SendDataBlock(const uint8_t *aData, uint32_t aLen)
{
    ...
    for (i = 0; i < aLen; ++i) {
        while (!UART1_SendByte(aData[i]));
    }
}
/*---------------------------------------------------------------------------*
 * Routine:  UART1_IsTransmitEmpty
 *---------------------------------------------------------------------------*
 * Description:
 *      Determine if all bytes in the transmit FIFO buffer of SCI2 have
 *      all been sent.
...
{
    /* Is the FIFO empty? */
    return G_UART1_TX_Empty;
}

/*---------------------------------------------------------------------------*
 * Interrupt Routine:  UART1_TX_ISRHandler
 *---------------------------------------------------------------------------*
 * Description:
 *      Interrupt handler to process bytes being sent from the transmit
 *      FIFO.
...
__interrupt void UART1_TX_ISRHandler(void)
{
    /* Clear the interrupt as the interrupt has been processed */
    STIF1 = 0U;	/* clear INTST0 interrupt flag */

    /* Is more data waiting to be sent? */
    if (G_UART1_TXIn != G_UART1_TXOut) {
        /* Send another byte and update the FIFO position */
        TXD1 = G_UART1_TXBuffer[G_UART1_TXOut++];
        if (G_UART1_TXOut >= UART1_TX_BUFFER_SIZE)
            G_UART1_TXOut = 0;
    } else {
        /* No data to send, mark transmitting as done. */
        /* This flag is needed so that the first byte is sent to */
...
/*---------------------------------------------------------------------------*
 * Interrupt Routine:  UART1_RX_ISRHandler
 *---------------------------------------------------------------------------*
 * Description:
 *      Interrupt handler processes incoming bytes and places them in the
 *      receive FIFO buffer.  If there is not enough room, the bytes
...
__interrupt void UART1_RX_ISRHandler(void)
{
    uint8_t c;
    uint32_t next;
    ...
    if (next >= UART1_RX_BUFFER_SIZE)
        next = 0;
    if (next != G_UART1_RXOut) {
        /* There is room to place the byte.  Place it and move up a byte. */
        G_UART1_RXBuffer[G_UART1_RXIn] = c;
        G_UART1_RXIn = next;
    } else {
        /* The buffer is overrunning and we are losing bytes now. */
    }
...
__interrupt void UART1_RXError_ISRHandler(void)
{
    /* log error */
    if(SSR03 & 1)
        G_UART1_LastError = FRAMING_ERROR;
    else if(SSR03 & 2)
        G_UART1_LastError = PARITY_ERROR;
    else if(SSR03 & 4)
        G_UART1_LastError = OVERRUN_ERROR;
    else
        G_UART1_LastError = UNKNOWN_ERROR;
    
    G_UART1_ErrorCount++;
    
    /* clear error flag */
    SIR03 = _SAU_SIRMN_FECTMN | _SAU_SIRMN_PECTMN | _SAU_SIRMN_OVCTMN;
