...
#define  M2MIO_USERNAME   	"54o9iri1st"        // "[your ThingFabric credentials username]"
#define  M2MIO_PASSWORD   	"1b00cb6e77322a2783afb05e6d246e86"        // "[your (pre-hashed) ThingFabric token]"
#define  M2MIO_DOMAIN     	"maaakihz"        // "[your domain]"
#define  M2MIO_STUFF    	"test-stuff"            // Was M2MIO_DEVICE_TYPE
#define  M2MIO_THINGS           "test-things"
#define M2MIO_BROKER_HOSTNAME   "q.thingfabric.com"
...
int m2mtest() {
	...
	sprintf(clientIDStr, "%s", myMac);
        ...
        DisplayLCD(LCD_LINE7, "Init Client");
        MSTimerDelay(2000);
	...
	// wait for CONNACK	
	packet_length = read_packet(10000);
	
        DisplayLCD(LCD_LINE7, "readpacket");
	...
	if(MQTTParseMessageType(rxm.message) != MQTT_MSG_CONNACK) { // MQTT_MSG_CONNACK       2<<4
            DisplayLCD(LCD_LINE7, "connack error");
            MSTimerDelay(2000);
            return -2;
	}
	...
	if(rxm.message[3] != 0x00) {
        ...
        sprintf(pubTopic, "%s/%s/%s", M2MIO_DOMAIN, M2MIO_STUFF, M2MIO_THINGS);
        mqtt_subscribe(&broker, pubTopic, NULL);
        
        DisplayLCD(LCD_LINE7, "Publishing!");
        MSTimerDelay(2000);
	...
	while (1) {
          ...
          if (G_receivedCount > 0) {
            ...
            if (MQTTParseMessageType(rxm.message) == MQTT_MSG_SUBACK) {
              DisplayLCD(LCD_LINE4, "MQTT Connect");  // don't expect this with QoS 0
		...
		if(updateRequest==FALSE)
          ...
          if (loop == 0) {
            ...
            addIntValToMsg("temp",temperature>>3, pubMsgStr);
