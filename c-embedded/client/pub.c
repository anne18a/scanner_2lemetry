...
int main(int argc, char* argv[])
{
	...
	mqtt_init(&broker, "embedded-example-client");
	mqtt_init_auth(&broker, "john@m2m.io", "0a7f108def6cd36309344dcf835cf3e8");
	...
	addStringValToMsg("status", "NORMAL", pubMsgStr);
	addIntValToMsg("speed", 100, pubMsgStr);
   addDoubleValToMsg("temperature", 20.65, pubMsgStr);
	finishJsonMsg(pubMsgStr);


	// >>>>> PUBLISH QoS 0
	printf("Publish: QoS 0\n");
	mqtt_publish(&broker, "io.m2m/myexample/example", pubMsgStr, 0);
